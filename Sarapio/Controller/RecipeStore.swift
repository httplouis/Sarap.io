// sarapio/Controller/RecipeStore.swift
import Foundation
...
final class RecipeStore: ObservableObject {
    ...
    func loadRecipes() async {
        ...
        do {
            ...
            print("❌ [RecipeStore] Load failed:", error)
            // Keep existing recipes if any
        }
        isLoading = false
    }
    ...
    func recipes(for user: AppUser?) -> [Recipe] {
        ...
        return recipes.filter { recipe in
            // Match by user_id or authorEmail
            // Also show recipes with null user_id (sample recipes) to all users
            recipe.user_id == user.id || 
            recipe.authorEmail == user.email ||
            (recipe.user_id == nil && recipe.authorEmail == nil)
        }
    }
